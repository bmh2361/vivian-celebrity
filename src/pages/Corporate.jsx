...
export default function Corporate() {
  ...
  const copy = {
    zh: {
      heroTitle: '企业策划 · Corporate Planning',
      ...
      heroDescEn:
        'Vivian Adventure provides one-stop corporate event and brand planning solutions. From concept to execution, we deliver impactful brand moments — product launches, corporate galas, media productions, and private events — all crafted with creativity and precision.',
      cta: '立即咨询 / Get in Touch',

      svcTitle: '服务范围 / Services',
      services: [
        { title: '品牌活动策划 / Brand Event Planning', desc: '提供从主题创意、流程设计到现场执行的全流程活动策划方案。', descEn: 'Plan and execute creative, customized corporate events from concept to completion.', icon: 'event' },
        { title: '企业影像制作 / Visual & Media Production', desc: '为企业制作宣传片、形象片、活动记录及品牌视觉内容。', descEn: 'Professional photography, video production, and promotional media for your brand.', icon: 'media' },
        { title: '品牌视觉升级 / Brand Identity Design', desc: '协助企业进行视觉体系优化与内容包装，增强品牌影响力。', descEn: 'Enhance your brand identity through refined design and visual storytelling.', icon: 'identity' },
        { title: '跨界联名策划 / Cross-Brand Collaboration', desc: '策划并执行品牌合作、展会及高端商务活动。', descEn: 'Build unique collaborations, exhibitions, and premium business events.', icon: 'link' },
      ],

      caseTitle: '成功案例 / Case Highlights',
      caseDesc: '我们为多家企业、品牌及机构提供了定制化的策划服务，涵盖从品牌发布到国际活动的多维场景。',
      cases: [
        { label: '企业年会布置 / Corporate Gala Setup', emoji: '📸' },
        { label: '新品发布会舞台 / Product Launch Stage', emoji: '🎥' },
        { label: '团队策划工作照 / Team Working Moments', emoji: '👥' },
        { label: '品牌视觉物料 / Brand Visual Design Samples', emoji: '🎨' },
        { label: '客户合影 / Client Collaboration Photo', emoji: '🤝' },
      ],

      flowTitle: '合作流程 / Our Process',
      steps: [
      ...
      partnerTitle: '合作伙伴 / Trusted by',
      quote: '“Vivian Adventure 的团队把我们的活动提升到了国际水准。” — 合作品牌代表',

      callTitle: '开启您的品牌旅程 / Start Your Next Chapter',
      callText: '让我们为您的企业打造专属的视觉与策划体验。联系我们，了解更多合作细节。\nLet’s craft something unforgettable together.',
      callBtn: '立即咨询 / Contact Us'
    },
    en: {
      heroTitle: 'Corporate Planning',
      heroSub: 'Strategy · Creative · Execution',
      heroDesc: [
        'Vivian Adventure offers one-stop brand planning and event execution.',
        'From product launches and corporate galas to media productions and private events,',
        'we craft memorable experiences with creativity and precision.',
      ],
      heroDescEn:
        'We deliver impactful brand moments — concept to execution — tailored for results.',
      cta: 'Get in Touch',

      svcTitle: 'Services',
      services: [
        { title: 'Brand Event Planning', desc: 'End-to-end planning: theme ideation, flow design and on-site execution.', descEn: '', icon: 'event' },
        { title: 'Visual & Media Production', desc: 'Corporate films, promos, photography and brand visual content.', descEn: '', icon: 'media' },
        { title: 'Brand Identity Design', desc: 'Identity optimization and content packaging for stronger presence.', descEn: '', icon: 'identity' },
        { title: 'Cross-Brand Collaboration', desc: 'Collaborations, exhibitions and premium business events.', descEn: '', icon: 'link' },
      ],

      caseTitle: 'Case Highlights',
      caseDesc: 'Selected custom projects spanning launches to international events.',
      cases: [
        { label: 'Corporate Gala Setup', emoji: '📸' },
        { label: 'Product Launch Stage', emoji: '🎥' },
        { label: 'Team Working Moments', emoji: '👥' },
        { label: 'Brand Visual Design Samples', emoji: '🎨' },
        { label: 'Client Collaboration Photo', emoji: '🤝' },
      ],
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-12">
      ...
      <motion.section {...fade(0)} className="rounded-2xl border border-[#eee] bg-white p-8 shadow-[0_8px_30px_rgba(0,0,0,0.05)] relative overflow-hidden">
        ...
        <div className="mt-3 text-[#555] space-y-1">
          ...
          <p className="text-[#666] mt-2">{t.heroDescEn}</p>
        </div>
        <div className="mt-5">
      ...
      <motion.section {...fade(0.1)}>
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {t.cases.map((c, i) => (
            <motion.div key={c.label} {...fade(i * 0.03)} className="relative rounded-2xl border border-[#eee] bg-white overflow-hidden shadow-[0_4px_18px_rgba(0,0,0,0.04)]">
              <div className="aspect-[4/3] grid place-items-center bg-gradient-to-br from-[#faf7ef] to-white text-3xl">{c.emoji}</div>
              <div className="px-4 py-3 text-sm text-[#444]">{c.label}</div>
            </motion.div>
          ))}
        </div>
      </motion.section>
      ...
      <motion.section {...fade(0.2)} className="rounded-2xl border border-[#eee] bg-white p-6 shadow-[0_4px_18px_rgba(0,0,0,0.04)]">
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-14 rounded-lg border border-[#eee] bg-[#fafafa] grid place-items-center text-xs text-[#888]">LOGO</div>
          ))}
        </div>
      </motion.section>
