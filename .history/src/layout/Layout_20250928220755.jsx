...
const dict = {
  zh: {
    nav: { home: '主页', makeup: '妆造', photography: '摄影', bespoke: '高端定制', portfolio: '作品集', about: '关于我们', contact: '联系我们' },
    cta: '预约咨询',
    footer: '© VIVIAN CELEBRITY. All rights reserved.'
  },
  en: {
    nav: { home: 'Home', makeup: 'Makeup', photography: 'Photography', bespoke: 'Bespoke', portfolio: 'Portfolio', about: 'About', contact: 'Contact' },
    cta: 'Consult',
    footer: '© VIVIAN CELEBRITY. All rights reserved.'
  }
};
...
// 语言上下文，供子页面读取
const LangContext = createContext({ lang: 'zh', t: dict.zh, setLang: () => {} });
export const useLang = () => useContext(LangContext);

export default function Layout({ pageKey, children }) {
  const [lang, setLang] = useState(() => {
    try {
      return typeof window !== 'undefined' && localStorage.getItem('lang') === 'en' ? 'en' : 'zh';
    } catch {
      return 'zh';
    }
  });
  ...
  const link = (href, label) => {
    ...
    return (
      <a href={href} className={`hover:text-[#111] ${isActive ? 'text-[#111] font-medium' : 'text-[#555]'}`}>
        {label}
      </a>
    );
  };
    ...
    <div className="min-h-screen flex flex-col bg-white text-[#111]">
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <a href="/pages/home.html" className="flex items-center gap-2 font-semibold tracking-widest text-[#CFAF6B]">
        ...
        {open && (
          <div className="md:hidden border-t border-[#e8e8e8] bg-white/95 backdrop-blur-sm">
            <div className="px-6 py-4 grid grid-cols-1 gap-3 text-sm">
              <a href="/pages/home.html" onClick={() => setOpen(false)} className="py-2">
                {t.nav.home}
              </a>
              <a href="/pages/makeup.html" onClick={() => setOpen(false)} className="py-2">
                {t.nav.makeup}
              </a>
              <a href="/pages/photography.html" onClick={() => setOpen(false)} className="py-2">
                {t.nav.photography}
              </a>
              <a href="/pages/bespoke.html" onClick={() => setOpen(false)} className="py-2">
                {t.nav.bespoke}
              </a>
              <a href="/pages/portfolio.html" onClick={() => setOpen(false)} className="py-2">
                {t.nav.portfolio}
              </a>
              <a href="/pages/about.html" onClick={() => setOpen(false)} className="py-2">
                {t.nav.about}
              </a>
              <a href="/pages/contact.html" onClick={() => setOpen(false)} className="py-2">
                {t.nav.contact}
              </a>
              <a
        ...
        {/* 底部金色分隔线（加粗） */}
        <div className="h-[3px] md:h-[4px] w-full bg-[#E6CF9A]" />
      </header>
