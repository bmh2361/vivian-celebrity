      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {pageKey !== 'contact' && (
              <a href={withBase('/pages/contact.html')} className="hidden md:inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-white bg-[#111] hover:bg黑色 transition-colors">
        ...
        {open && (
          <div className="md:hidden border-t border-[#e8e8e8] bg-white/95 backdrop-blur-sm">
            <div className="px-6 py-4 grid grid-cols-1 gap-1 text-base">
              <a href="/pages/home.html" onClick={() => setOpen(false)} className="py-3 hover:text-[#CFAF6B] transition-colors">
                {t.nav.home}
              </a>
              <a href={withBase('/pages/home.html')} onClick={() => setOpen(false)} className="py-3 hover:text-[#CFAF6B] transition-colors">
                {t.nav.makeup}
              </a>
              <a href={withBase('/pages/makeup.html')} onClick={() => setOpen(false)} className="py-3 hover:text-[#CFAF6B] transition-colors">
                {t.nav.photography}
              </a>
              <a href={withBase('/pages/photography.html')} onClick={() => setOpen(false)} className="py-3 hover:text-[#CFAF6B] transition-colors">
                {t.nav.bespoke}
              </a>
              <a href={withBase('/pages/bespoke.html')} onClick={() => setOpen(false)} className="py-3 hover:text-[#CFAF6B] transition-colors">
                {t.nav.portfolio}
              </a>
              <a href={withBase('/pages/portfolio.html')} onClick={() => setOpen(false)} className="py-3 hover:text-[#CFAF6B] transition-colors">
                {t.nav.about}
              </a>
              <a href={withBase('/pages/about.html')} onClick={() => setOpen(false)} className="py-3 hover:text-[#CFAF6B] transition-colors">
                {t.nav.contact}
              </a>
              <a href={withBase('/pages/contact.html')} onClick={() => setOpen(false)} className="py-3 hover:text-[#CFAF6B] transition-colors">
                href="/pages/contact.html"
                onClick={() => setOpen(false)}
                className="inline-flex items-center justify-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-white bg-[#111] hover:bg-black transition-colors"
