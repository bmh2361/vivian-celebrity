...
function MarqueeColumn({ images, reverse }) {
	// 使用三份队列 + 新 keyframe marquee-y-3，避免顶部与底部出现空白窗口
	const cls = `flex flex-col gap-4 marquee-runner marquee-allow-motion`;
	const tripled = [...images, ...images, ...images];
	return (
		<div className="overflow-hidden rounded-xl border border-[#eee] pt-0 px-2 pb-2 bg-white/70">
			<div
				className={cls}
				style={{
					['--gap']: '0.9rem', // 略缩小间距，增加填充感
					animationName: 'marquee-y-3',
					animationDuration: 'var(--dur, 90s)', // 更慢，三份内容整体循环
			...
			>
				{tripled.map((src, i) => (
					<img
						...
						decoding="async"
						className="w-full max-w-full h-auto object-cover rounded-lg bg-[#f2f2f2]"
					/>
				))}
			</div>
		</div>
