...
function MarqueeColumn({ images, reverse }) {
	// 绝对定位 runner + 限高容器，避免底部滚动出现巨大空白
	const doubled = [...images, ...images];
	const runnerRef = useRef(null);
	const [colH, setColH] = useState(null); // 单份高度
	const loadedRef = useRef(0);

	const onErr = (e) => { e.currentTarget.style.display = 'none'; };
	const onLoad = () => {
		loadedRef.current++;
		if (loadedRef.current === images.length && runnerRef.current) {
			// runner 总高度是双份，取一半
			const total = runnerRef.current.scrollHeight;
			if (total) setColH(total / 2);
		}
	};

	return (
		<div
			className="relative overflow-hidden rounded-xl border border-[#eee] pt-0 px-2 pb-2 bg-white/70"
			style={{ height: colH ? colH + 'px' : '60vh' }}
		>
			<div
				ref={runnerRef}
				...
				style={{
					['--gap']: '1rem',
					animationName: colH ? 'marquee-tight' : 'none', // 等高度测量后再启用动画
